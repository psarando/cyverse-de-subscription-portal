...
type QueueMessage = {
    ...
    CustomAction?: React.JSXElementConstructor<any>;
};
...
const CyVerseAnnouncer = () => {
    ...
    useEffect(() => {
        let timer = setInterval(tickCallback, TIMEOUT);
    ...
    const dequeue = () => {
        if (msgQueue.length > 0) {
            ...
            if (msgQueue.length === 0 && timeout !== EMPTY_QUEUE_TIMEOUT) {
                clearInterval(timer);
                let newTimer = setInterval(tickCallback, EMPTY_QUEUE_TIMEOUT);
                setTimer(newTimer);
                setTimeout(EMPTY_QUEUE_TIMEOUT);
            } else if (timeout !== TIMEOUT) {
                clearInterval(timer);
                let newTimer = setInterval(tickCallback, TIMEOUT);
                setTimer(newTimer);
                setTimeout(TIMEOUT);
            }
        }
